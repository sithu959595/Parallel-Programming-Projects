...
const char *			CL_FILE_NAME = { "Project7cl.cl" };
...
//NUM_ELEMENTS = Size;
	...
	float *hC = new float[ numWorkGroups ];        //Mul
	...
	{
		hA[i] = hB[i] = (float) sqrt( (double)i  );   // (double)i  
              //  hD[i]=(float) sqrt(  (double)i  ); //Multiply-Add
              //  hD[i]=2.0;                //Multiply        	
}
        size_t signaldataSize=Size*sizeof(float);
	...
	cl_mem dA = clCreateBuffer( context, CL_MEM_READ_ONLY,  tSize, NULL, &status );
	if( status != CL_SUCCESS )
		fprintf( stderr, "clCreateBuffer failed (1)\n" );



	cl_mem dD = clCreateBuffer( context, CL_MEM_READ_ONLY,  sSize, NULL, &status );
        if( status != CL_SUCCESS )
	...
	status = clEnqueueWriteBuffer( cmdQueue, dA, CL_FALSE, 0, tSize, T, 0, NULL, NULL );
	if( status != CL_SUCCESS )
		fprintf( stderr, "clEnqueueWriteBuffer failed (1)\n" );

	
/*float hD=2.;
        status = clEnqueueWriteBuffer( cmdQueue, dD, CL_FALSE, 0, iSize, &hD, 0, NULL, NULL );
        if( status != CL_SUCCESS )
...
/*	int c=0;
	...
	fprintf( stderr, "%8d\t%4d\t%10d\t%10.12lf MegaMultsPerSecond++++++%8.6lf\n",
		NMB, LOCAL_SIZE, NUM_WORK_GROUPS, (double)NUM_ELEMENTS/(time1-time0)/1000000.,sum );
...
fprintf(stderr,"Wrong and difference is %10.6lf and index is %d \n",d,in);
}
c=0;      */
#ifdef WIN32
	Sleep( 2000 );
